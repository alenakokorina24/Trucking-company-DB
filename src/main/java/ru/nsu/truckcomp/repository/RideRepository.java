...
public interface RideRepository extends CrudRepository<Ride, Integer> {
    ...
    List<Mileage> getTransportDayMileage(int id, Date day);

    @Query(value = "SELECT new ru.nsu.truckcomp.model.Mileage(ri.transport.id, SUM(ri.route.routeLength)) FROM Ride ri " +
            "WHERE ri.transport.id = ?1 AND TO_CHAR(ri.date, 'YYYY-MM') = ?2 " +
            "GROUP BY ri.transport.id")
    List<Mileage> getTransportMonthMileage(int id, String month);

    @Query(value = "SELECT new ru.nsu.truckcomp.model.Mileage(ri.transport.id, SUM(ri.route.routeLength)) FROM Ride ri " +
            "WHERE ri.transport.id = ?1 AND YEAR(ri.date) = ?2 " +
            "GROUP BY ri.transport.id")
    List<Mileage> getTransportYearMileage(int id, int year);
    ...
    @Query(value = "SELECT new ru.nsu.truckcomp.model.Mileage(ri.transport.id, SUM(ri.route.routeLength)) FROM Ride ri " +
            ...
            "WHERE ri.date = ?1 " +
            "GROUP BY ri.transport.id")
    List<Mileage> getPassengerTransportDayMileage(Date day);

    @Query(value = "SELECT new ru.nsu.truckcomp.model.Mileage(ri.transport.id, SUM(ri.route.routeLength)) FROM Ride ri " +
            "INNER JOIN PassengerTransport p ON p.id=ri.transport.id " +
            "WHERE TO_CHAR(ri.date, 'YYYY-MM') = ?1 " +
            "GROUP BY ri.transport.id")
    List<Mileage> getPassengerTransportMonthMileage(String month);

    @Query(value = "SELECT new ru.nsu.truckcomp.model.Mileage(ri.transport.id, SUM(ri.route.routeLength)) FROM Ride ri " +
            "INNER JOIN PassengerTransport p ON p.id=ri.transport.id " +
            "WHERE YEAR(ri.date) = ?1 " +
            "GROUP BY ri.transport.id")
    List<Mileage> getPassengerTransportYearMileage(int year);
    ...
    @Query(value = "SELECT new ru.nsu.truckcomp.model.Mileage(ri.transport.id, SUM(ri.route.routeLength)) FROM Ride ri " +
            ...
            "WHERE ri.date = ?1 " +
            "GROUP BY ri.transport.id")
    List<Mileage> getTruckDayMileage(Date day);

    @Query(value = "SELECT new ru.nsu.truckcomp.model.Mileage(ri.transport.id, SUM(ri.route.routeLength)) FROM Ride ri " +
            "INNER JOIN Truck t ON t.id=ri.transport.id " +
            "WHERE TO_CHAR(ri.date, 'YYYY-MM') = ?1 " +
            "GROUP BY ri.transport.id")
    List<Mileage> getTruckMonthMileage(String month);
